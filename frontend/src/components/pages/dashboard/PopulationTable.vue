        ...
        <div class="max-h-[580px] overflow-y-auto">
          ...
          >
            <span class="flex-grow whitespace-nowrap p-2 font-semibold capitalize">
              <router-link
                :to="{ name: 'PatientDetails', params: { id: patient.fhir_id } }"
                class="hover:underline"
                @click="handleClick({ id: patient.fhir_id })"
              >
...
const filteredPatients = computed(() => {
  return patientsStore.items.filter((patient) => {
    return patient.full_name.toLowerCase().includes(patientQuery.value.toLowerCase())
  })
})
...
const handlePatientDetails = (patient) => {
  vaccineModalIndex.value = patient.fhir_id
