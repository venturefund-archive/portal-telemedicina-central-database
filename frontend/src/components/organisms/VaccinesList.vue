            ...
            <table class="w-full table-auto text-left tracking-wide md:table-fixed lg:table-fixed border-separate">
              ...
              <tbody class="border-y-2 border-[#F8F9FB]">
                <tr
                  ...
                  v-for="(vaccine, k) in filteredVaccines"
                  :key="k"
                >
                  <td colspan="2" class="relative cursor-default rounded-l-full bg-[#F1F1F1] px-5 py-1 text-sm">
                    <Tooltip variant="blue" position="default">
                      ...
                      <template #content>
                        <!-- tooltip -->
                        <div class="flex justify-center">{{ vaccine.display}}</div>
                      </template>
                    </Tooltip>
                  </td>
                    ...
                    <div v-for="(dose, dk) in filteredDosesByVaccine(vaccine)" :key="dk" class="flex justify-center">
                      <div class="group relative">
                        <VaccineAlert
                          :vaccine="vaccine"
                          :dose="dose"
                          v-if="dose.forecast_status && dose.forecast_status === 'complete'"
                        ...
                        <div v-else-if="dose.forecast_status === 'overdue'">
                          <VaccineAlert
                            :vaccine="vaccine"
                            :dose="dose"
                            :rangeIndex="rangeIndex"
                        ...
                        >
                          <div class="flex items-center space-x-2 p-3">
                            <template
                              v-if="
                                dose.forecast_status && dose.forecast_status === 'complete'
                              "
                            >
                              <svg
                                xmlns="http://www.w3.org/2000/svg"
                              ...
                              <div>
                                ...
                                <span>{{ format(parseISO(dose.due_date), 'dd/MM/yyyy') }}</span>
                              </div>
                            </template>
                            <!-- <template
                              v-else-if="
                            ...
                            <template v-else-if="dose.forecast_status && dose.forecast_status === 'overdue'">
                              <span class="relative inline-flex h-8 w-8 rounded-full bg-red-500">
                                <span
                            ...
                            <template v-else>
                              ...
                              <div>
                                <p class="text-base font-bold">
                                  {{ dose.vaccine_dose.dose_order }}º {{ $t('patient-details.dose') }}
                                </p>
                                <span
...
const filteredVaccines = computed(() => {
  return vaccinesStore.items.filter((vaccine) => {
    return (
      vaccine.system == 'BRI' &&
      dosesStore.items.map((e) => e.vaccine_dose.vaccine.code).includes(vaccine.code) &&
...
const filteredDosesByVaccine = computed(() => {
  return (vaccine) =>
    dosesStore.items.filter((dose) => {
      return dose.vaccine_dose.vaccine.code == vaccine.code
    })
