<template>
    ...
    <ul class="absolute bg-white p-3 w-full rounded"
    v-if="suggestions.length && ! hideSuggestions">
        ...
        >
            <router-link :to="{ name: 'Patient', params: {id: suggestion.id} }" class="hover:underline">
                <div class="flex items-center gap-4 p-4">
                    <img class="w-12 h-12 rounded-full" :src="suggestion.image" />
                    <span class="text-slate-900 text-sm font-medium dark:text-slate-200">{{ suggestion.name }}</span>
                </div>
            </router-link>
        </li>
    </ul>
