<template>
  <router-link v-if="to" :to="!disabled ? to : null" :class="classes" :aria-disabled="disabled.toString()">
    <span v-if="srText" class="sr-only">{{ srText }}</span>
    <slot :iconSizeClasses="iconSizeClasses" />
  </router-link>
  <a v-else-if="href" :href="!disabled ? href : null" :class="classes" :aria-disabled="disabled.toString()">
    <span v-if="srText" class="sr-only">{{ srText }}</span>
    <slot :iconSizeClasses="iconSizeClasses" />
  </a>
...
const props = defineProps({
  variant: {
    ...
    default: 'primary',
    validator(value) {
      return ['primary', 'secondary', 'success', 'success-outline', 'danger', 'warning', 'info', 'black'].includes(value)
    },
  },
  type: {
    type: String,
...
const variantClasses = (variant) => ({
  ...
  'bg-white border border-2 border-green-500 text-green-500 hover:text-white hover:bg-[scondary-color] focus:ring-green-500': variant == 'success-outline',
